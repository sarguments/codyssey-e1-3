...
필터 B도 점수와 판정을 구할 때 사용하지만 성능 측정에는 포함하지 않으며, 표의 연산 횟수는 필터 하나의 MAC을 기준으로 `3² = 9`이다.
...
*두 모드는 모두 같은 `measure_mac()`으로 입출력을 제외한 필터 하나의 MAC을 10회 반복 측정한다.*
...
|---|---|---|
| 측정 목적 | 직접 입력한 3×3 MAC 시간 확인 | 행렬 크기에 따른 MAC 시간 비교 |
| 측정 함수 | `measure_mac()` | `measure_mac()` |
| 측정 데이터 | 입력한 패턴과 필터 A | 3×3 예제와 크기별 첫 번째 패턴·Cross 필터 |
| 입력 크기 | 3×3 고정 | 3×3, 5×5, 13×13, 25×25 |
| 출력 시간 | 필터 A MAC 10회 평균 | 크기별 Cross MAC 10회 평균 |
| 연산 횟수 기준 | 필터 하나의 `N²` | 필터 하나의 `N²` |
